...
imgs = [img.get_fdata().transpose(2, 0, 1) for img in imgs]

lbl_files = glob(LBL_PATH + files)[:num_of_imgs]
lbls = [nib.load(file) for file in lbl_files]
lbls = [lbl.get_fdata().transpose(2, 0, 1) for lbl in lbls]

train_data = DataSet(np.array(imgs), np.array(lbls))

patch_length = 128
patch_size = (3, patch_length, patch_length)
patchExtractor = PatchExtractor(patch_size, flipping=False)
target_size = (3, patch_length, patch_length)
batch_size = 10
...
# # Testing examples
#
# example_file = "pancreas_001.nii.gz"
# img = nib.load(IMG_PATH + example_file)
# img = img.get_fdata().transpose(2, 0, 1)
#
# label = nib.load(LBL_PATH + example_file)
# label = label.get_fdata().transpose(2, 0, 1)
#
# print(label.shape)
#
# datas = DataSet(np.array([img]), np.array([label]))
#
# patch_length = 512
# patch_size = (3, patch_length, patch_length)
# patchExtractor = PatchExtractor(patch_size, flipping=False)
...
# batch_size = 1
#
# batchCreator = BatchCreator(patchExtractor, datas, target_size)
#
# gen = batchCreator.get_image_generator(batch_size)
#
#
